<!--
## Hi there 👋

**nivash-kumar/nivash-kumar** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
# 👋 Hi, I'm Nivash Kumar  

🚀 A passionate and creative **Web Developer** dedicated to crafting visually appealing and highly functional websites.  
💻 Currently pursuing **BCA at Patliputra University, Patna**.  
🌱 Exploring **C Programming, Data Structures, Algorithms, and Frontend Development**.  
🌐 Learning **MERN Stack (MongoDB, Express.js, React.js, Node.js)** for full-stack web development.  
🎨 Also practicing **UI/UX Design in Figma** and focusing on responsive, user-friendly designs.  

---

## 🛠️ Skills & Tools  

- **Languages:** C, JavaScript, HTML, CSS  
- **Frontend Frameworks:** Tailwind CSS, React.js  
- **Backend & Databases:** Node.js, Express.js, MongoDB  
- **Design Tools:** Figma,
<!-- - **Other Interests:** Data Structures, Algorithms, AutoCAD, Vector Analysis  -->

---

<!-- ## 📂 Featured Projects  

🔹 [Portfolio Website](#) – My personal portfolio built with HTML, Tailwind, and JavaScript.  
🔹 [Course Seller Website](#) – A responsive landing page design for SkillUp.  
🔹 [ISRO Mission Records](#) – A card-based UI to showcase ISRO missions with details.  

*(I’ll keep updating this space as I add more projects!)*  

---   -->

## 📊 GitHub Stats  

![Nivash's GitHub stats](https://github-readme-stats.vercel.app/api?username=nivash-kumar&show_icons=true&theme=tokyonight)  
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=nivash-kumar&layout=compact&theme=tokyonight)  

---

## 🌐 Connect with Me  

- 📧 Email: **nivash.code@gmail.com**  
- 💼 LinkedIn: [[www.linkedin.com/in/nivash-kumar](https://www.linkedin.com/in/nivash-kumar/)](#)  
<!-- - 🌍 Portfolio: [nivashkumar.dev](#) --> 

---

✨ *"I love turning ideas into real-world digital experiences."*  
